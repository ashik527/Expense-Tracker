...
export const generateMonthlyInsight = async ({
    ...
    const trendText = previousMonths.length > 0
        ? previousMonths
            .map(m => `- ${m.month}: Income ${currency} ${m.income.toFixed(2)}, Expenses ${currency} ${m.expense.toFixed(2)}`)
            .join('\n')
...
export const generateSavingsTips = async ({
    ...
    const categoryText = topCategories.length > 0
        ? topCategories
            .map(c => `- ${c.category}: ${currency} ${c.amount.toFixed(2)} across ${c.transactionCount} transactions`)
            .join('\n')
...
Return ONLY valid JSON (no markdown):
{
  ...
  "highlight": "Single short phrase capturing the key takeaway (e.g., 'Heavy on dining', 'Stable income', 'Dining down')"
}`;
...
export const analyzeBudgetList = async ({
    ...
    const prompt = `You're a personal finance assistant. Analyze each budget below and provide a one-sentence status and recommendation.
